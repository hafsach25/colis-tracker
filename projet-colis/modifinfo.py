...
class Modifinf(ctk.CTkFrame):
    ...
    #frame pour les infos
        ...
        self.ville_label.place(x=730, y=210)
        self.ville_menu = ctk.CTkOptionMenu(self, variable=self.selected_ville)
        self.ville_menu.place(x=770, y=210)
...
######################################################################################
    ...
    def submit_form(self):
        ...
        if not username  or not country or not city or not address or not email or not phone :
    ...
    # Message de confirmation
        ...
        if soc:
           c.execute("update societé set adr_societ=:adr, num_telsoc=:num ,email_societ=:email, ville=:vil, pays=:pays  where ice_societ=:id",{'adr':address,
                                                                                                                                     'num':phone,
                                                                                                                                     'email':email,
                                                                                                                                     'vil':city,
                                                                                                                                     'pays':country,
            ...
            c.execute("update individu set adr_individu=:adr, numTel_individu=:num, email_individu=:email, ville=:vil, pays=:pays  where cin_individu=:id",{'adr':address,
                                                                                                                                     'num':phone,
                                                                                                                                     'email':email,
                                                                                                                                     'vil':city,
                                                                                                                                     'pays':country,
    ...
    def insertent(self):
             ...
             if l1[0][1] is None:
                 ...
                 soc=False
             if soc:
               c.execute("select ville,pays,adr_societ,email_societ,num_telsoc from societé where ice_societ=?",(l1[0][2],))
               l2=c.fetchall()
               print(l2)
             else:
               c.execute("select ville,pays,adr_individu,email_individu,numTel_individu from individu where cin_individu=?",(l1[0][1],))
               l2=c.fetchall()
               print(l2)
             
             self.usr_entry.insert(0,l1[0][0])
             self.pays_menu.set(l2[0][1])
             self.ville_menu.set(l2[0][0])
             self.addr_entry.insert(0,l2[0][2])
             self.email_entry.insert(0,l2[0][3])
             self.phone_entry.insert(0,l2[0][4])
